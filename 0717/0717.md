...
- gbif 데이터에서 lat과 lon컬럼 추출하기
    ...
    pinus_df <- pinus_densiflora[, c('country', 'lat', 'lon')]
    ```
- lat과 lon의 값이 있는 경우만 추출하기
    ```R
    valid_pinus_df <- pinus_df[!is.na(pinus_df$lat) & !is.na(pinus_df$lon), ]
    ```
    lat이나 lon중 NA가 있는 데이터를 빼버렸더니, 총 4115개의 수목데이터가 1597개로 줄었다.
