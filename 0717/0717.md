...
- 우리가 하려고 하는 parsing 작업!
    ...
    - lat과 lon의 범위가 이상한 경우 제대로 만들어주기
    ...
    - 국가별 제대로 된 위도/경도 정보 얻기
        - `geocode()` 함수 사용
            ...
            - 예) Peru의 위치 정보 얻기
                ...
                ## xmax(위도 max), xmin(위도 min)
                ## ymax(경도 max), ymin(경도 min)
                peru_loc <- peru_geo[, c('longitude', 'latitude', 'xmin', 'xmax')]
                print(peru_loc)
                ```
                출력 결과
                ```
                ...
                [9] "uncertainty"
                  longitude  latitude     xmin      xmax     ymin       ymax
                1 -75.01515 -9.189967 -81.3867 -68.65233 -18.4483 -0.0387769
                ```
