...
- 예) treedb의 occurrence 데이터에 대한 기후 변수값 얻기
    ...
    for(num in 1:19){
        col<-paste('bio',num,sep='')
        df2[,col] <- treedbextract[,col]
        #df2$col<-treedbextract$col
    }
    ...
    df3[, c('bio1', 'bio2')] <- treedbextract[, c('bio1', 'bio2')]
    df3[,cols]<-treedbextract[,]


    plot(worldclim, 1)
    map(database = 'world', region = c('South Korea', 'North Korea'))
    points(df3[df3$kind=='Chionanthus retusus',]$lon, df3[df3$kind=='Chionanthus retusus',]$lat, col='blue')
    points(df3[df3$kind=='Taxus cuspidata',]$lon, df3[df3$kind=='Taxus cuspidata',]$lat, col='red')
