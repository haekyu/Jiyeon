...
- `predict()` 함수
    ...
    - 예2) 오늘 한 내용 복붙 (아직 정리 전)
        ...
        treedf<-extract(climraster,treedb[,c(3,2)])
        treedf <- treedf[treedf$kind == 'Chionanthus retusus',]

        model<-bioclim(treedf[,1:3])


        rcpraster<-getData('CMIP5', download = TRUE, var='bio', res=10, rcp=85, year=70, model='AC')


        rcpdf<-extract(rcpraster,treedb[,c(3,2)])

        
        rcpmodel<-bioclim(rcpdf[,1:3])

        japanextent<-geocode('japan')
        japanextent<-japanextent[,3:4]

        japandf<-extract(climraster,japanextent)
        japanrcpdf<-extract(rcpraster,japanextent)
        as.data.frame(japanrcpdf)[1:3]
        japanrcpdf<-as.data.frame(japanrcpdf)
        japanrcpdf<-japanrcpdf[,1:3]


        japandf<-japandf[,1:3]
        japandf <- data.frame(cbind(94, 107, 29))
        colnames(japandf) <- c('bio1', 'bio2', 'bio3')
        # japandf2<-as.data.frame(japandf)
        japanpd<-predict(model, x=japandf)
        japanrcp_pd<-predict(rcpmodel, x=japanrcpdf)
...
### 3.3 Maxent 모델로 예측하기
- `predict()` 함수 사용
- 예) 한국 데이터로 fitting 시킨 maxent 모델로 이팝나무(Chionanthus retusus)가 일본의 어느 곳에 살만한지 예측해보기 (아직 정리 전)
    ```R
    library(dismo)
    climraster<-getData('worldclim', download = TRUE, var='bio', res=10)
    ...
    library('rJava')
    mx <- maxent(climraster, as.data.frame(treedb[,c(3,2)]))
