...
### 3.3 Maxent 모델로 예측하기
- `predict()` 함수 사용
- 예1) 한국 이팝나무(Chionanthus retusus) 데이터로 maxent 모델 fitting 시키기
    ```R
    ################################################################
    # 한국 데이터 준비
    ################################################################
    ...
    mx <- mx <- maxent(climraster[[1:3]], retusus_lon_lat)
    ...
    ```
    위 출력과 함께 다음과 같은 [html 페이지](maxent_retusus_123.html)가 뜬다.
